...
function Article() {
  ...
  return (
    <div className="bg-gray-300 px-3 py-4 min-w-48 min-h-96 w-full">
      {articleEditStatus === false ? (
        <div className="main">
          <div className="flex justify-between">
            <div className="">
              <p className="py-2 text-2xl font-mono font-semibold">
                {state.title}
              </p>
              <div className="flex gap-4">
                <p className="text-sm">
                  <FcCalendar className="text-xl inline pe-1" />
                  Created on {ISOtoUTC(state.dateOfCreation)}
                </p>
                <p className="text-sm">
                  <FcClock className="text-xl inline pe-1" />
            ...
            <div className="">
              {currentUser.userType === "author" && (
                <div className="flex gap-2">
                  <button
                    onClick={enableEdit}
                    className="border border-black rounded bg-orange-300 px-1 py-0.5"
                  >
                    <CiEdit className="text-xl inline" />
                  </button>
                  {currentArticle.status === true ? (
                    <button
                      onClick={deleteArticle}
                      className="border border-black rounded bg-red-400 px-1 py-0.5"
                    >
                      <MdDelete className="text-xl inline" />
                    </button>
                  ) : (
                    <button
                      onClick={restoreArticle}
                      className="border border-black rounded bg-green-400 px-1 py-0.5"
                    >
                      <MdRestore className="text-xl inline" />
                    </button>
                  )}
                </div>
              )}
          ...
          <p className="py-4 pe-4" style={{ whiteSpace: "pre-line" }}>
            {state.content}
          </p>
          {/* comments */}
          ...
          <div className="">
            ...
            <div className="py-3">
              {currentUser.userType === "user" && (
                ...
                <form onSubmit={handleSubmit(addComment)}>
                  <input
                    type="text"
                    className="w-96 sm:w-5/6 md:w-4/6 lg:w-3/6 xl:w-2/6 py-1 ps-2 pe-1 text-md border-2 border-black rounded "
                    placeholder="Add a comment..."
                    {...register("comment", { required: true })}
                  />
                  <button type="submit" className="rounded px-1 py-0.5">
                    <BiSend className="text-3xl inline" />
                  </button>
                </form>
              )}
            </div>
            ...
            {comment !== null && (
              <div className="w-full sm:w-5/6 md:w-4/6 lg:w-3/6 xl:w-2/6 bg-slate-100 m-1 px-2 py-1 text-lg rounded-lg">
                ...
                >
                  <FcPortraitMode className="text-xl inline pe-1" />{" "}
                  {comment.username}
                </p>
                <p
                  className="w-full"
                ...
                >
                  <FcComments className="text-xl inline pe-1" />
                  {comment.comment}
                </p>
              </div>
            )}
            <div className="">
              {state.comments && state.comments.length === 0 ? (
                ...
                state.comments.map((commentObj, ind) => (
                  <div
                    key={ind}
                    className="w-full sm:w-5/6 md:w-4/6 lg:w-3/6 xl:w-2/6 bg-slate-100 m-1 px-2 py-1 text-lg rounded-lg"
                  >
                    <p
                      className="w-full"
                      style={{
